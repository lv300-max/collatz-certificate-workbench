...
QTRANSITION = os.environ.get("QPARENT_QTRANSITION_TABLE", "quotient_transition_table.json")
...
def key_id(key):
    return json.dumps(key_list(key), separators=(",", ":"))
...
def parent_coverage_source(status):
    if status == "CLOSED_BY_QUOTIENT":
        return "quotient_closed"
    if status == "CLOSED_WITH_LOCAL_KEYS":
        return "local_key_closed"
    if status == "OPEN_KEYS_COVERED_BY_EXISTING_FRONTIER":
        return "existing_frontier_covered"
    if status == "COVERED_BUT_CAP_REACHED":
        return "covered_open_keys_despite_cap"
    return "unresolved_or_incomplete"


def normalize_parent_row(row):
    if "k_prime" in row:
        row.setdefault("modulus", f"2^{row['k_prime']}")
    row.setdefault("coverage_source", parent_coverage_source(row.get("status")))
    row.setdefault("sampled", False)
    row.setdefault("exact", True)
    return row


def exact_witness_has_full_state(item):
    ...
    return final_b is not None and int(final_b) <= eqa.B_LIMIT


def closed_quotient_witness_export_complete(row):
    witnesses = row.get("exact_closure_witnesses", [])
    if not witnesses:
        return False
    if len(witnesses) != int(row.get("entry_count", -1)):
        return False
    if len(witnesses) != int(row.get("return_keys", -1)):
        return False
    return all(exact_witness_has_full_state(item) for item in witnesses)


def local_examples_need_exact_state(row):
...
def enrich_local_key_examples(row):
    if not row.get("local_key_examples") or not local_examples_need_exact_state(row):
        return row
    payloads = getattr(enrich_local_key_examples, "_transition_payloads", None)
    if payloads is None:
        payloads = transition_table_local_payloads()
        setattr(enrich_local_key_examples, "_transition_payloads", payloads)
    if payloads:
        enriched_from_table = []
        all_found = True
        for item in row.get("local_key_examples", []):
            payload = payloads.get(key_tuple(item["key"]))
            if payload is None:
                all_found = False
                break
            enriched_from_table.append(attach_local_exact_payload(dict(item), payload))
        if all_found:
            row["local_key_examples"] = enriched_from_table
            row["local_key_exact_state_exported"] = True
            return row

    parent = (int(row["r0"]), int(row["k_prime"]), int(row["depth"]))
    entries, _per_parent = eqa.find_entries([parent])
    wanted = {key_tuple(item["key"]) for item in row.get("local_key_examples", [])}
    representatives = find_representatives_for_keys(entries, wanted)
    enriched = []
    for item in row.get("local_key_examples", []):
        key = key_tuple(item["key"])
        state = representatives.get(key)
        if state is None:
            enriched.append(item)
            continue
        updated = local_key_continuation(key, state, "targeted_exact_state_backfill")
        enriched.append(updated)
    row["local_key_examples"] = enriched
    row["local_key_exact_state_exported"] = all(
        isinstance(item.get("exact_state"), dict) for item in enriched
    )
    return row
...
def summarize_rows(rows, total_parent_count):
    ...
    return {
        ...
        "closed_by_quotient_full_state_exports": sum(
            1 for row in rows
            if row.get("status") == "CLOSED_BY_QUOTIENT"
            and row.get("closed_by_quotient_full_state_export_complete") is True
        ),
        "closed_by_quotient_exact_witnesses": sum(
            int(row.get("exact_closure_witness_count", 0) or 0)
            for row in rows
            if row.get("status") == "CLOSED_BY_QUOTIENT"
        ),
    }
...
def find_representatives_for_keys(entries, wanted):
    queue = deque()
    representatives = {}
    found = {}
    visits = 0
...
def attach_local_exact_payload(record, payload):
    record["exact_state"] = payload
    record["residue"] = payload["residue"]
    record["r"] = payload["r"]
    record["modulus_power"] = payload["modulus_power"]
    record["k"] = payload["k"]
    record["modulus"] = payload["modulus"]
    record["o"] = payload["o"]
    record["c"] = payload["c"]
    record["b"] = payload["b"]
    record["gap"] = payload["gap"]
    record["gap_sign"] = payload["gap_sign"]
    record["initial_B"] = payload["B"]
    record["exact"] = True
    record["sampled"] = False
    if record.get("classification") == "HIGH_B_RETURN_THEN_CERTIFIED":
        record["final_B"] = record.get("b_control_next", {}).get("B")
    else:
        record["final_B"] = record.get("B")
    record["terminal_outcome"] = record.get("classification")
    return record


def attach_local_exact_state(record, state):
    return attach_local_exact_payload(record, exact_state_payload(state))


def reconstruct_payload_from_key_affine(key, affine):
    ...
    return {
        ...
        "sampled": False,
        "residue_low128": str(residue & ((1 << min(128, level)) - 1)),
        "reconstruction": "exact inverse of exported affine/key congruence",
    }


def transition_table_local_payloads():
    report = maybe_load_json(QTRANSITION) or {}
    out = {}
    for row in report.get("rows", []):
        ...
        for rep in row.get("representatives", []):
            if rep.get("representative_source") != OUTFILE:
                continue
            payload = reconstruct_payload_from_key_affine(key, rep.get("affine_state") or {})
            if payload is not None:
                out[tuple(key)] = payload
                break
    return out
...
def local_key_continuation(key, state, reason):
    ...
    for _ in range(LOCAL_KEY_PREFIX_STEPS):
        ...
        if ret is not None:
            if ret["B_ok"]:
                return attach_local_exact_state({
                    "key": key_list(key),
        ...
        if len(successors) != 1:
            ...
            if zero_ret.get("classification") == "CONFLICT":
                return attach_local_exact_state({"key": key_list(key), **zero_ret}, state)
            if zero_ret["B_ok"]:
                return attach_local_exact_state({
                    "key": key_list(key),
                    "classification": "CERTIFIED_RETURN",
...
def exact_entry_closure_witness(parent_row, state, entry_index):
    initial_state = state
    initial_key = eqa.quotient_key(initial_state)
    cur = state
    prefix = ""
    for _ in range(eqa.MAX_STEPS + 1):
        ret = eqa.classify_positive(cur)
        if ret is not None:
            return_key = eqa.quotient_key(cur)
            base = {
                "witness_id": (
                    f"{parent_row['r0']}:{parent_row['k_prime']}:{parent_row['depth']}:"
                    f"{entry_index}:{key_id(initial_key)}"
                ),
                "parent_r0": int(parent_row["r0"]),
                "parent_k_prime": int(parent_row["k_prime"]),
                "parent_depth": int(parent_row["depth"]),
                "parent_modulus": f"2^{int(parent_row['k_prime'])}",
                "entry_index": entry_index,
                "key": key_list(initial_key),
                "return_key": key_list(return_key),
                "successor_key": key_list(return_key),
                "classification": "CERTIFIED_RETURN" if ret["B_ok"] else "HIGH_B_RETURN_THEN_CERTIFIED",
                "reason": "closed parent entry certifies from its own exact affine state",
                "coverage_source": "FULL_EXACT_STATE_CERTIFICATE",
                "prefix": prefix,
                "prefix_length": len(prefix),
                "successor_return_path": prefix,
                "return_pair": list(ret["pair"]),
                "B": ret["B"],
                "final_B": ret["B"],
                "exact": True,
                "sampled": False,
                "terminal_outcome": "CERTIFIED_RETURN" if ret["B_ok"] else "HIGH_B_RETURN_THEN_CERTIFIED",
                "return_state": exact_state_payload(cur),
                "successor_state": exact_state_payload(cur),
            }
            if not ret["B_ok"]:
                next_ret = one_then_zero_return(cur[5], cur[4], cur[3])
                base["b_control_next"] = next_ret
                base["final_B"] = next_ret.get("B")
                base["classification"] = (
                    "HIGH_B_RETURN_THEN_CERTIFIED"
                    if next_ret.get("B_ok")
                    else "NEEDS_HIGHER_LOCAL_CAP"
                )
                base["terminal_outcome"] = base["classification"]
            return attach_local_exact_state(base, initial_state)
            ...
            return attach_local_exact_state(
                {
                    ...
                    "key": key_list(initial_key),
                    "classification": "CONFLICT",
                    "reason": "exact entry path branches before a certified return",
                    "coverage_source": "INCOMPLETE_FULL_STATE_EXPORT_FOR_CLOSED_BY_QUOTIENT",
                    "prefix": prefix,
                    "prefix_length": len(prefix),
                    "exact": True,
                    "sampled": False,
    ...
    return attach_local_exact_state(
        {
            ...
            "entry_index": entry_index,
            "key": key_list(initial_key),
            "classification": "STILL_DEBT_LOCAL_CAP",
            "reason": "exact entry path did not reach a certified return under existing step cap",
            "coverage_source": "INCOMPLETE_FULL_STATE_EXPORT_FOR_CLOSED_BY_QUOTIENT",
            "prefix": prefix,
            "prefix_length": len(prefix),
            "exact": True,
...
def enrich_closed_quotient_witnesses(row):
    if row.get("status") != "CLOSED_BY_QUOTIENT":
        return row
    if closed_quotient_witness_export_complete(row):
        row["closed_by_quotient_full_state_export_complete"] = True
        row["closed_by_quotient_export_status"] = "PASS_FULL_STATE_EXPORT_FOR_CLOSED_BY_QUOTIENT"
        row["coverage_source"] = "FULL_EXACT_STATE_CERTIFICATE"
        return row

    parent = (int(row["r0"]), int(row["k_prime"]), int(row["depth"]))
    entries, _per_parent = eqa.find_entries([parent])
    witnesses = [
        exact_entry_closure_witness(row, state, index)
        for index, (_r0, _kv, _depth, state) in enumerate(entries)
    ]
    row["exact_closure_witnesses"] = witnesses
    row["exact_closure_witness_count"] = len(witnesses)
    row["exact_closure_witness_classification_counts"] = dict(
        Counter(item.get("classification") for item in witnesses)
    )
    row["exact_closure_witness_max_final_B"] = max(
        (int(item["final_B"]) for item in witnesses if item.get("final_B") is not None),
        default=None,
    )
    row["exact_closure_witness_max_prefix_length"] = max(
        (int(item.get("prefix_length", 0)) for item in witnesses),
        default=0,
    )
    complete = closed_quotient_witness_export_complete(row)
    row["closed_by_quotient_full_state_export_complete"] = complete
    row["closed_by_quotient_export_status"] = (
        "PASS_FULL_STATE_EXPORT_FOR_CLOSED_BY_QUOTIENT"
        if complete
        else "INCOMPLETE_FULL_STATE_EXPORT_FOR_CLOSED_BY_QUOTIENT"
    )
    row["coverage_source"] = (
        "FULL_EXACT_STATE_CERTIFICATE"
        if complete
        else "INCOMPLETE_FULL_STATE_EXPORT_FOR_CLOSED_BY_QUOTIENT"
    )
    return row
...
def classify_parent(parent, covered_sets):
    ...
    return {
        ...
        "modulus": f"2^{parent['k_prime']}",
        "depth": parent["depth"],
        "status": status,
        "coverage_source": parent_coverage_source(status),
        "sampled": False,
        "exact": True,
...
def write_report(rows_by_id, all_parents, started_at, cap_info, final=False):
    rows = [
        normalize_parent_row(
            enrich_closed_quotient_witnesses(
                enrich_local_key_examples(rows_by_id[str(parent["r0"])])
            )
        )
